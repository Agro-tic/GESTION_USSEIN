  ...
  <div class="page-heading mb-4">
    <div class="page-heading-copy">
      ...
      <div>
        ...
        <h1 class="h3 mb-1">Modifier le Congé</h1>
        <p class="text-muted mb-0">Modifiez les informations du congé</p>
      </div>
    </div>
  </div>
      ...
      <div class="row g-3">
          ...
          <select class="form-select" name="agent_id" id="agent_id" required>
            ...
            @foreach($agents as $agent)
              <option value="{{ $agent->id }}"
                {{ old('agent_id', $conge->agent_id) == $agent->id ? 'selected' : '' }}>
        ...
        <div class="col-md-3">
          ...
          <input class="form-control" type="date" name="date_cessation" id="date_cessation"
                 value="{{ old('date_cessation', $conge->date_cessation) }}" required>
        ...
        <div class="col-md-3">
          ...
          <input class="form-control" type="number" name="jours_a_prendre" id="jours_a_prendre"
                 value="{{ old('jours_a_prendre', $conge->jours_a_prendre) }}"
        ...
        <div class="col-md-3">
          ...
          <input class="form-control bg-light" type="date" name="date_reprise" id="date_reprise"
                 value="{{ old('date_reprise', $conge->date_reprise) }}" readonly>
          <small class="text-muted">Calculée automatiquement</small>
        </div>

        <div class="col-md-3">
          <label for="jours_restants" class="form-label fw-semibold">Jours restants</label>
          <input class="form-control bg-light" type="number" name="jours_restants" id="jours_restants"
                 value="{{ old('jours_restants', $conge->jours_restants) }}" readonly>
        ...
        <div class="col-md-6">
          ...
          <select class="form-select" name="statut" id="statut">
            <option value="en_attente" {{ old('statut', $conge->statut) == 'en_attente' ? 'selected' : '' }}>
              En attente
            </option>
            <option value="approuve" {{ old('statut', $conge->statut) == 'approuve' ? 'selected' : '' }}>
              Approuvé
            </option>
            <option value="refuse" {{ old('statut', $conge->statut) == 'refuse' ? 'selected' : '' }}>
              Refusé
            </option>
          </select>
        </div>
